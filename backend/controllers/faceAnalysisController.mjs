...
export const deleteAnalysis = async (req, res) => {
  try {
    ...
    if (analysis.images.front) {
      const frontPath = path.join(process.cwd(), analysis.images.front);
      if (fs.existsSync(frontPath)) {
        fs.unlinkSync(frontPath);
      }
    }
    if (analysis.images.left) {
      const leftPath = path.join(process.cwd(), analysis.images.left);
      if (fs.existsSync(leftPath)) {
        fs.unlinkSync(leftPath);
      }
    }
    if (analysis.images.right) {
      const rightPath = path.join(process.cwd(), analysis.images.right);
